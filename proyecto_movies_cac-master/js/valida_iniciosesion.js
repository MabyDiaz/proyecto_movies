    ...
    if (!validateForm()) {
      ...
      try {
        ...
        const response = await fetch(
          'https://proyecto-movies-7vlw.onrender.com/login',
          {
            method: 'POST',
            headers: {
        ...
        if (data.token) {
          ...
          // Redireccionar al usuario a la página principal (index.html) después de iniciar sesión
          window.location.href = data.redirectTo || '/index.html';

          // Actualizar la interfaz de usuario para mostrar el enlace de Cerrar Sesión
